...
namespace WiFiMessenger.Platforms.iOS.Touch
{
    ...
    public async Task HandleTouch( TouchStatus status, TouchInteractionStatus? interactionStatus = null )
    {
      ...
      await UIView.AnimateAsync( .2, () =>
      {
        if( color == default( Color ) )
          ...
          renderer.Layer.BackgroundColor = Microsoft.Maui.Platform.ColorExtensions.ToCGColor( isStarted ? color : control.BackgroundColor );
